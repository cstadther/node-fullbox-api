...
function uploadFile(values, options, callback) {
  findFile(values.filename, values.folderid, options, function(data) {
  ...
  var req = request(header, function(e,r,body) {
	...
	if(results.total_count > 1) {
	  ...
	  for(var i = 0; i < results.total_count; i++) {
	    if(results.entries[i].name == require('path').basename(file) && results.entries[i].parent.id == folderid) {
	      found = true;
